...
/**
 * 프로시험때 제출했던 코드를 기억해 의존해서 작성했습니다.
 * 실제 제출했을 때는 동작이 제대로 안 될 수 있음을 양해 부탁드립니다.
 */
public class UserSolution {
    ...
    public int add(String word) {
        // 아마 이미 추가한 적이 있는 단어가 주어졌다면 0을 반환하고 아니면 1을 반환하라는 문제였을 겁니다..
        if (wordMap.containsKey(word)) return 0;
        wordList.add(getIndex(word, wordList), createWord(word)); // 해당 인덱스에 단어를 넣는다.
        // 만약 1번 wordList.add(1, 넣고자하는 단어) 연산을 수행하면 1번 인덱스에 그 단어가 들어가고 원래 1번이었던 단어부터 뒤의 단어들은 모두 한칸씩 뒤로 밀려난다.
        for (int i = getAlphabetIndex(word.charAt(0)) + 1; i < alphabetLen; i++) {
    ...
    public int delete(String word) {
        // 이것도 아마 실패하면 0 반환, 성공하면 1반환하면서 삭제였던 것 같습니다
        if (wordMap.containsKey(word) || wordMap.get(word).isRemoved) return 0;
        Word target = wordMap.get(word);
        target.isRemoved = true;
        wordList.remove(getIndex(word, wordList));
        for (int i = getAlphabetIndex(word.charAt(0)) + 1; i < alphabetLen; i++) {
    ...
    public Result find(char alphabet, int index) {
        // 기억이 가물가물한데.. alphabet으로 시작하는 단어 중 index번째 단어가 존재한다면 그 단어를 Result.word에 넣어서 반환하라는 문제였던 것 같습니다.
        Result result = new Result();
        int alphabetIndex = getAlphabetIndex(alphabet);
        if (frontIdx[alphabetIndex + 1] - frontIdx[alphabetIndex] < index) {
        ...
        result.word = wordList.get(frontIdx[alphabetIndex] + index).word;
        return result;
    }

    public int getIndex(String word) {
        // 주어진 단어가 몇 번째 단어인지 구하라는 메서드였던 것 같습니다.
        if (!wordMap.containsKey(word) || wordMap.get(word).isRemoved) return 0; // 존재하지 않다면 0을 반환하라고 했던 것 같습니다 아마..

        // 인덱스를 구하는 게 아니라 몇 번째인지를 구하는 문제기 때문에 +1을 해서 반환
        return getIndex(word, wordList) + 1;
    }

    // Result 클래스는 Main Class 내부에서 주어집니다.
    static class Result {
        int success;
        String word;
    }
